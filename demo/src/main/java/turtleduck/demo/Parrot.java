...
public class Parrot {
	...
	public boolean draw(TurtleDuck turtle) {
		...
		register(turtle.child().changePen().strokePaint(Colors.FORESTGREEN).strokeWidth(5).done().turn(180).draw(.8*w).mark("tail"));
...
//		upTurtle.strokeAndFillPath(Colors.GRAY.darker(), Colors.GRAY.opacity(0.5));
		...
		TurtleDuck sub = turtle.child().turn(60).move(.25 * w).turn(-60);
			...
			sub.changePen().strokeWidth(3).done().shape().width(.35 * w).height(.25 * h).strokePaint(Colors.GREY).ellipse().draw();
...
/*		if(code.equals(KeyCode.B)) {
			if(shift)
				beakAngle = Math.max(0,  beakAngle-5);
			else
				beakAngle = Math.min(120,  beakAngle+5);
		} else if(code.equals(KeyCode.N)) {
			if(shift)
				neckAngleStep1 = Math.max(-50,  neckAngleStep1 - 0.5);
			else
				neckAngleStep1 = Math.min(50,  neckAngleStep1 + 0.5);
		} else if(code.equals(KeyCode.M)) {
			if(shift)
				neckAngleStep2 = Math.max(-50,  neckAngleStep2 - 0.5);
			else
				neckAngleStep2 = Math.min(50,  neckAngleStep2 + 0.5);
		}
		*/
	}
