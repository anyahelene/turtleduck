		...
		try {
			...
			if (data.python) {
				...
				if (msg_type === 'hello') {
					...
					port.postMessage({
						header: { msg_type: 'welcome', ref_id: msg_id, msg_id: "r" + msg_id },
					...
					port.postMessage({
						header: { msg_type: 'init_python_reply', ref_id: msg_id, msg_id: "r" + msg_id },
						content: { status: 'ok' }
					});
					...
					self.pyodide.runPythonAsync("ShellService.receive()", self.pymsg, self.pyerr).then(res => {
						...
						if (res instanceof Map) {
							if (!(res.has('content') && res.has('header'))) {
								res = {
									header: { msg_type: msg_type + '_reply', ref_id: msg_id, msg_id: "r" + msg_id },
									content: res
								};
							} else {
