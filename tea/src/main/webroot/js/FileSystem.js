	...
	return db.transaction('rw', [db.files],
		() => {
			if (path.next > 0) {
				...
				const last = path;
				return db.files.add({
					name: path.name, parent: path.parent, data: newData,
...
class DBFileSystem extends FileSystem {
	...
	_create_impl(name, parent, kind, rev) {
		...
		return this.files.add({
			name: name, data: null, parent: parent,
...
class VirtFileSystem extends FileSystem {
	constructor(cwd = 0, project = undefined) {
		...
		this.files = {
			0:
				new VirtFile({
					inode: 0, name: '', data: null, parent: 0,
					modtime: new Date(), kind: 'd', ver: 0, files: {}
				}, this)
		};
		this.nextInode = -1;
	...
	_list_impl(file, filter) {
		...
		const fileList = [];
		for (const name in file.files) {
			fileList.push(file.files[name]);
		}
	...
	_create_impl(name, parent, kind, rev) {
		...
		const file = new VirtFile({
			inode: inode, name: name, data: null, parent: parent,
			kind: kind, next: 0, ver: rev, modtime: new Date()
		}, this);
		if (kind === 'd')
			file.files = {};
...
var dbFileSystem;
const fileSystem = new VirtFileSystem();
...
export { fileSystem, FileSystem, dbFileSystem };
