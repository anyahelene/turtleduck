...
class HistoryDexie extends Dexie {
	...
	constructor() {
		...
		this.version(6).stores({
			...
			await tx.table("historyIds").toCollection().each(async (entry) => {
				...
				await this.sessions.put({ session: key[0], shell: key[1], id: entry.historyId, date: new Date(), shells: [] });
			});
			return await tx.table("historyEntries").toCollection().each(async (entry_1) => {
				const key_1 = make_key(entry_1.session);
				await this.entries.put({ session: key_1[0], shell: key_1[1], id: entry_1.historyId, data: entry_1.line });
			});
		});
...
export enum SearchDirection { BACKWARDS = -1, FORWARDS = 1 };
export interface HistorySession {
	search(query: string, dir?: SearchDirection, skip?: boolean): Promise<string | undefined>;
	init(): Promise<void>;
...
class DBHistorySession implements HistorySession {
	...
	async search(query: string, dir: SearchDirection = SearchDirection.BACKWARDS, skip = false): Promise<string | undefined> {
		const id = skip ? this.id + dir : this.id;
		const entry = dir === SearchDirection.BACKWARDS //
			? await db.entries.where(this.qKey).and(e => e.id < id && e.data.indexOf(query) > 0).reverse().first() // 
			: await db.entries.where(this.qKey).and(e => e.id > id && e.data.indexOf(query) > 0).first()
		if (entry) {
			this.go(entry);
			return this.line;
		} else {
	...
	async enter(): Promise<number> {
		this.edits = [];
		return DBHistory.put(this.session, this.line).then(id => { this.id = id; return Promise.resolve(id); });
	}
	private go(entry: Entry) {
		if (entry) {
			this.id = entry.id;
			if (this.edits[this.id] === undefined)
				this.line = entry.data;
			else
				this.line = this.edits[this.id];
	...
	async prev(): Promise<string> {
		this.go(await db.entries.where(this.qKey).and(e => e.id < this.id).reverse().first());
		return this.line;
	}
	async next(): Promise<string> {
		this.go(await db.entries.where(this.qKey).and(e => e.id > this.id).first());
		return this.line;
	}
}
const DBHistory: History = {
	...
	async get(session: string, id?: number): Promise<string> {
		...
		return await db.entries.get(make_key(session, id))
	...
	put(session: string, data: string, id?: number): Promise<number> {
		...
		return db.transaction('rw', [db.sessions, db.entries],
			() => db.sessions.get(key).then(async x => {
				const latest = x ? { ...x, date: new Date() } : { session: key[0], shell: key[1], id: 0, shells: [], date: new Date() };
				...
				await db.entries.put({ session: key[0], shell: key[1], id: thisId, data: data });
...
class FakeHistorySession implements HistorySession {
	...
	async search(query: string, dir?: SearchDirection, skip?: boolean): Promise<string> {
	...
	async enter(): Promise<number> {
		this.edits = [];
		return FakeHistory.put(this.session, this.line).then(id => { this.id = id; return Promise.resolve(id); });
	}
	async first(): Promise<string> {
		throw new Error('Method not implemented.');
	}
...
const FakeHistory = {
	...
	async get(session: string, id?: number): Promise<string> {
		...
		if (!fakeHistoryEntries[session])
			fakeHistoryEntries[session] = [];
		if (typeof id != 'number') {
			id = await this.currentId(session);
		}
	...
	put(session: string, data: string, id?: number): Promise<number> {
		...
		fakeHistoryEntries[session][thisId] = { id: thisId, data: data, session: key[0], shell: key[1] };
		return Promise.resolve(thisId);
	},
