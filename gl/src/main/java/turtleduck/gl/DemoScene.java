...
public class DemoScene implements TurtleDuckApp {
...
//				System.out.println("bearing: " + bearing() + ", rotation: " + rotation + ", azimuth: " + bearing().azimuth());
	...
	public void bigStep(double deltaTime) {
		...
		turtle.spawn().jumpTo(600, 100).turnTo(0).strokeWidth(1).stroke(Color.color(.5, .5, .5, 1)).draw(200).turn(45)
				.draw(200).turn(90).draw(200).turn(-90).draw(200);
//		if(true)
//			return;

		fern(turtle.spawn().stroke(Colors.GREEN).goTo(200, 200).turnTo(135), 10, 5);
		fern2(turtle.spawn().goTo(900, 200).turnTo(90), 5, 10);
		tree(turtle.spawn().goTo(850, 200).stroke(Color.color(.5, .3, .0)).strokeWidth(2).turnTo(90), 10, 10);
		wheel1(turtle.spawn().goTo(500, 200).turnTo(-2 * rotation).stroke(Color.color(0.5, 0, 0, 0.5)));
...
//			img = img.crop(50, 50, 200, 200);
	...
	public static void main(String[] args) {
		new GLLauncher().app(new DemoScene()).launch(args);
	}
...
//					System.out.println(turtle.bearing().degrees());
	...
	private void fernX(Turtle turtle, double h, int n) {
		double factor = 1;
		if (n <= 0) {
			turtle.strokeWidth(2).stroke(Colors.YELLOW);
	...
	private void fern2(Turtle turtle, double h, int n) {
		if (n <= 0)
			...
			turtle.strokeWidth(1).stroke(Colors.GREEN);
			turtle.draw(h * n * 2);
		} else {
	...
	private void tree(Turtle turtle, double h, int n) {
		...
		turtle.stroke(Color.color(.4, 1, 0).mix(Color.color(.4, .25, 0), stage / 3));
		if (n <= 1)
			turtle.stroke(Colors.RED).draw(Math.min(stage, 1) * 5);
		else {
