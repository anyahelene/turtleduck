...
public class Stats {
	...
	static class Stat {
		...
		void print(String title, String unit, boolean invert) {
			double c = sincePrint / Math.max(1, nSincePrint);
			...
			System.err.printf("%-8s: cur=%8.2f %-4s, avg=%8.2f %-4s, ravg=%8.2f %-4s, min=%8.2f %-4s, max=%8.2f %-4s    ", title,
					c * factor, unit, (total / n) * factor, unit, ra * factor, unit, mn * factor, unit, mx * factor, unit);
