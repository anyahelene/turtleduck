...
public class Point2 implements Point {
    ...
    public Direction directionTo(PositionVector otherPoint, Direction dflt) {
        double dx = otherPoint.x() - x(), dy = otherPoint.y() - y();
        double l = dx * dx + dy * dy;
        if (l < 10e-9)
            return dflt;
        else
            return Direction.absolute(dx, dy);
    }
    ...
    public Point add(DirectionVector dir, double distance) {
        // TODO: return new Point2(Math.fma(dir.dirX(), distance, x),
        // Math.fma(dir.dirY(), distance, y));
        if (dir.dirZ() != 0)
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PositionVector)) {
            return false;
        }
