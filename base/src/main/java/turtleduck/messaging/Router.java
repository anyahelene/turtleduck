...
public class Router {
    ...
    public void connectGroup(Connection conn, String... groups) {
        String connName = conn.id();
        if (!connections.containsKey(connName)) {
            throw new IllegalStateException("unknown connection: " + connName);
        } else {
            for (String g : groups) {
                List<Connection> list = connectionGroups.get(g);
                if (list == null)
    ...
    public void receive(Message msg) {
        ...
        if (msg.msgType().equals("error_reply")) {
            if (DEBUG)
                logger.info("Received error reply to unknown message {}: {}", msg.msgRef(), msg);
            return;
        }
