...
//	protected List<TurtleImpl<THIS, RESULT>> children = new ArrayList<>();
    ...
    public TurtleImpl(TurtleImpl<?, ?> parent, Function<TurtleImpl<THIS, RESULT>, RESULT> result) {
        super(parent);
        this.pen = parent.pen != null ? parent.pen : new BasePen();
        this.parent = parent;
        this.writer = parent.writer;
        this.result = result;
        this.objId = parent.objId != null ? parent.objId : "path@" + System.identityHashCode(this);
        this.canvas = parent.canvas;
    }

    public TurtleImpl(Point p, Direction b, Pen pen, Canvas canvas) {
        super(p, b);
        this.pen = pen != null ? pen : new BasePen();
        this.canvas = canvas;
...
//		Path path = Path.fromList(points);
    ...
    public THIS pen(Pen newPen) {
        pen = newPen != null ? newPen : new BasePen();
        penBuilder = null;
        return (THIS) this;
    }
    ...
    @SuppressWarnings("unchecked")
    @Override
    public THIS goTo(PositionVector newPos) {
        super.goTo(newPos);
        triggerActions();
        return (THIS) this;
    }
    ...
    @SuppressWarnings("unchecked")
    @Override
    public THIS jumpTo(PositionVector newPos) {
        drawing = false;
        boolean penStatus = penDown;
        penDown = false;
        goTo(newPos);
