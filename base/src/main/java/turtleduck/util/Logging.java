...
import turtleduck.util.impl.LoggerWrapper;
...
public class Logging {
    ...
    public static Logger getLogger(String name) {
        getFactory();
        if (factory != null) {
            return new LoggerWrapper(factory.getLogger(name));
        } else {
    ...
    public static Logger getLogger(Class<?> clazz) {
        getFactory();
        if (factory != null) {
            return new LoggerWrapper(factory.getLogger(clazz.getName()));
        } else {
    ...
    static class CustomLogger implements Logger {
        ...
        private int prevLevel = LOG_LEVEL_INFO;
        ...
        public void debugEnabled(boolean enable) {
            int lvl = enable ? Math.min(prevLevel, LOG_LEVEL_DEBUG) : Math.max(prevLevel, LOG_LEVEL_INFO);
            prevLevel = level;
            level = lvl;
        }
