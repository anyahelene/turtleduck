...
public class TShell {
	...
	public TShell(Screen screen, Printer printer) {
		this.screen = screen;
...
//		builder.fileManager(fm -> new FileManager(fm));
		...
		printer.println("testValue = " + testValue);
	...
	public boolean doCompletion() {
		if (completions != null) {
			...
			for (int i = compY; i < printer.getY(); i++)
				printer.clearLine(i);
			printer.moveTo(compX, compY);
		...
		int perLine = Math.max(1, printer.getLineWidth() / (maxLen[0] + 2));
		int n = 0;
		printer.println();
