...
class DragState {
    ...
    constructor() {
        ...
        this._handler = (ev: DragEvent) => { this.updateDrag(ev); };
    }
    ...
    startDrag(ev: DragEvent, dragSource: HTMLElement, dragStyle: string) {
        ...
        if (dragStyle === 'live') {
            this.offsets = { startX: ev.pageX, startY: ev.pageY, dragging: true };
    ...
    resumeDrag(dropTarget: HTMLElement, ev: DragEvent) {
        if (this.offsets && !this.offsets.dragging && this.dragSource && this.dragSource !== dropTarget) {
...
function targetElement(tgt: EventTarget): HTMLElement {
    ...
    if (elt.nodeType === Node.TEXT_NODE) { // might actually be a Text node
...
function leaveDropTarget(ev: DragEvent, newTgt?: HTMLElement) {
    ...
    if (dragState.dropTarget) {
        if (newTgt)
            console.log("DragNDrop LEAVE", ev.type, dragState.dropTarget, "TO", newTgt, ev);
        else
...
function enterDropTarget(tgt: HTMLElement, ev: DragEvent, oldTgt: HTMLElement) {
    if (tgt) {
        ...
        dragState.dropAllowed = 'none';
        if (oldTgt)
            console.log("DragNDrop ENTER", ev.type, dragState.count, tgt, "FROM", oldTgt, ev);
        else
            console.log("DragNDrop ENTER", ev.type, dragState.count, tgt, ev);
...
export function attachDropZone(elt: HTMLElement, query: string = '[data-drop]') {
...
function dragstart(ev: DragEvent) {
    if (dragState.dropTarget || dragState.count !== 0) {
        console.error("dragstart with existing dropTarget or count !== 0", dragState.dropTarget, dragState.count);
        dragState.reset();
    }
    if (ev.currentTarget instanceof HTMLElement) {
        ...
        ev.dataTransfer.setDragImage(elt, elt.offsetWidth / 2, -elt.offsetHeight / 2);
...
function dragenter(ev: DragEvent) {
    ...
    const drop = dropTarget(target, dropZones.get(ev.currentTarget as HTMLElement));
    if (!drop) {
...
function dragleave(ev: DragEvent) {
    ...
    if (!drop || drop !== dragState.dropTarget) {
        console.log("ignored   dragleave", dragState.count, drop, "!==", dragState.dropTarget, ev, target);
        return;
...
function drop(ev: DragEvent) {
    ...
    if (dragState.dragSource)
        console.log("DragNDrop DROP ELEMENT", dragState.dragSource, dragState.dropTarget, ev);
    else
        console.log("DragNDrop DROP DATA   ", ev.dataTransfer, dragState.dropTarget, ev);
...
const self = {
    DragState, dragState, attachDraggable, attachDropZone, detachDraggable, detachDropZone, BorbDragEvent,
