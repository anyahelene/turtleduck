...
class MDRender {
  ...
  render_unsafe(elt, code) {
    ...
    if (toc) {
      ...
      elt
        .querySelectorAll('div[data-level="2"], div[data-level="1"]')
  ...
  _render_link_open(old) {
    return function (tokens, idx, options /* env */) {
      ...
      const title = link.title
        ? ` title="${utils.escapeHtml(utils.replaceEntities(link.title))}"`
        : '';
  ...
  _render_image(old) {
    return function (tokens, idx, options /*, env */) {
      ...
      var alt = ` alt="${
        tokens[idx].alt
          ? utils.escapeHtml(
              utils.replaceEntities(utils.unescapeMd(tokens[idx].alt)),
            )
          : ''
      }"`;
